 ...
 */
Authorization.prototype.generateCode = function(options, next) {
 ...
 */
Authorization.prototype.checkCode = function(options, next) {
  var self = this;
  
  // get the data related to the issued code and delete any reference from the grants store 
  this.grants.del(options.code, function(err, grant) {
    if (err) return next(err);
    if (!grant) return next(new Error('no such grant found'));
    
    //
    // TODO: Check issued_at date with current date and be sure that time difference <10 min (see 4.1.2)
    //
  
    return next(null, grant.user_id, grant.client_id, grant.scope);  
  
    if (options.redirect_uri) {
    }
    
    // check client coupling using client_id or redirect_uri
    if (options.client_id === grant.client_id) {
