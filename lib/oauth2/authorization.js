 ...
 */
Authorization.prototype.generateCode = function(user_id, client_id, next) {
 ...
 */
Authorization.prototype.checkCode = function(client_id, code, next) {
  var self = this;
  
  // get the user that issued the grant code
  this.grants.get(code, function(err, grant) {
    if (err) return next(err);
    if (client_id === grant.client_id) {
      self.grants.del(code, function(err) {
        if (err) return next(err);
 ...
 */
Authorization.prototype.generateAccessToken = function(user_id, client_id, next) {
  var access_token = this.serializer.stringify([user_id, client_id, +new Date()]);
  return next(null, access_token);
};
