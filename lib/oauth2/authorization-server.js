...
AuthorizationServer.prototype.authorizationEndpointGet = function(req, res, next) {
  ...
  if (redirect_uri) {
    redirect_uri = this.getRedirectURI(redirect_uri, client_id);
  } else {
    // return with invalid_uriclient error 4.2.2.1
    return this.authError_invalid_uriclient(req, res);
  }

  // check validity of response_type 
  if (response_type) {
    this.getResponseType(response_type);
  } else {
    // return with unsupported_response_type 4.2.2.1 
    return this.authError_unsupported_response_type(req, res, {redirect_url: redirect_url});
  }

  // authentication server will return to this GET endpoint with the same parameters
  // authorization form will be POSTed to the same URL with the same parameters
  ...
  this.enforceLogin(req, res, authorize_url, function(user_id) {
    ...
    // user is logged in, render approval page for client and scope authorization
    self.authorizeForm(req, res, client_id, authorize_url, scope);
  });
}
 ...
 */
AuthorizationServer.prototype.authorizeForm = function(req, res, client_id, cb_url, scope) {
  //
