 ...
 * @param {Function} next The callback function called with the clean redirect_uri or `undefined` if the redirect_uri
 * was not accepted.
 */
...
AuthorizationServer.prototype.checkClient = function(headers, options, next) {
  ...
  this.clients.lookup(options.client_id, function(err, client) {
    ...
    if (client.type === 'confidential' && client.secret === options.client_secret) {
      ...
      //
      if (client.type === 'confidential') return next(Error("Wrong secret!"));
      if (client.secret) return next(Error("Client secret provided for a 'public' client!"));
      return next(null);
    }
  });
};
