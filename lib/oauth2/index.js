...
oauth2.Authentication = require('./plugin/authentication');
oauth2.Authorization = require('./plugin/authorization');
...
/**
 * initialize the oauth2 module with 
...
oauth2.init = function(options) {
  ...
  if (options.authentication) {
    oauth2.authentication = new oauth2.Authentication(options.authentication);
  }
  
  // initialize the authorization plugin
  if (options.authorization) {
    oauth2.authorization = new oauth2.Authorization(options.authorization);
  }

  // initialize the authorization server
  if (options.authorizationServer) {
    oauth2.authentication = options.authorizationServer.authentication = options.authorizationServer.authentication || oauth2.authentication;
    oauth2.authorization = options.authorizationServer.authorization = options.authorizationServer.authorization || oauth2.authorization;
    oauth2.authorizationServer = new oauth2.AuthorizationServer(options.authorizationServer);
  }
  
  // initialize the resource server
  if (options.resourceServer) {
    oauth2.resourceServer = new oauth2.ResourceServer(options.resourceServer);
  }
...
/**
 * Initialize a OAuth2 URL router (Director) with the correct endpoints. If no Director router is given and Flatiron
 * Director is included as package then that is used as router else return null
 * @param {Object} options Options for the router to create
 */
oauth2.createRouter = function(options) {
  ...
  // check if router is given and if not check if Director is included and if not return undefined
  if (options.router) {
...
/**
 * Call the middleware functions that executes OAuth2 actions for this Resource Server. This function needs to be called
 * by a middleware accepter like wrapping the http module or using Union/Connect/Express.
 */
