 ...
 *   - eSecret {String} The encryption secret to used for creating Access tokens (default: 'ThisIsAnEncryptionSecret') 
 *   - sSecret {String} The signing secret to used for signing stuff (default: 'ThisIsASigningSecret')
 */
 ...
 *
 * This function checks if an access token was received in a query parameter or HTTP header (the `authorization` header
 * with `Bearer` as start, as defined by the OAuth2 spec http://www.ietf.org/id/draft-ietf-oauth-v2-bearer-14.txt). If
 * an access token is found it calls this.accessToken with the decrypted access token data for further processing.
...
ResourceServer.prototype.checkToken = function(req, res, next) {
  ...
  if (req.query.access_token) {
    ...
    // check if Bearer type else go to next middleware
    if (req.headers.authorization.indexOf('Bearer ') === -1) return next();
...
/**
 * An access token was received in a URL query string parameter or HTTP header so save information to be used by the
 * service in req.token. Overwrite this function to have your own processing of the access token in relation to the
 * token creating Authorization Server.
